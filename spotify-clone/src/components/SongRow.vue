...
<template>
    <li
        class="flex items-center justify-between rounded-md hover:bg-[#000]"
    ...
    >
        <div class="flex items-center w-full py-1.5">
            ...
            <div v-else class="text-white font-semibold w-[40px] ml-5">
                <span :class="{'text-pink-500': currentTrack && currentTrack.name === track.name}">
                    {{ index }}
                </span>
            </div>
            <div>
                <div
                    :class="{'text-pink-500': currentTrack && currentTrack.name === track.name}"
        ...
        <div class="flex items-center">
            <button type="button" v-if="isHover">
                <Heart fillColor="#ef0061" :size="22"/>
            </button>
            <div
                v-if="isTrackTime"
